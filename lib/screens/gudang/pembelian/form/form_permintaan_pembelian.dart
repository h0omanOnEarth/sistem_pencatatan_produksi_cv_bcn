...
class FormPermintaanPembelianScreen extends StatefulWidget {
  ...
  const FormPermintaanPembelianScreen(
      {Key? key, this.purchaseRequestId, this.materialId, this.statusPRQ})
      : super(key: key);
...
class _FormPermintaanPembelianScreenState
    extends State<FormPermintaanPembelianScreen> {
  ...
  void initializeMaterial() {
    ...
    firestore
        ...
        .where('id',
            isEqualTo: selectedKodeBahan) // Gunakan .where untuk mencocokkan ID
        .get()
        .then((QuerySnapshot querySnapshot) async {
      if (querySnapshot.docs.isNotEmpty) {
        final bahanData =
            querySnapshot.docs.first.data() as Map<String, dynamic>;
        final namaProduk = bahanData['id'];
  ...
  void addOrUpdate() {
    ...
    final purchaseRequeset = PurchaseRequest(
        id: '',
        catatan: catatanController.text,
        jumlah: int.tryParse(jumlahProdukController.text) ?? 0,
        materialId: selectedKodeBahan ?? '',
        satuan: selectedSatuan,
        status: 1,
        statusPrq: statusController.text,
        tanggalPermintaan: _selectedDate ?? DateTime.now());

    if (widget.purchaseRequestId != null) {
      purchaseRequestBloc.add(UpdatePurchaseRequestEvent(
          widget.purchaseRequestId ?? '', purchaseRequeset));
    } else {
                          ...
                          children: [
                            Expanded(
                              child: ElevatedButton(
                                onPressed: widget.statusPRQ == "Selesai"
                                    ? null
                                    : () {
