...
class FormPengubahanBahan extends StatefulWidget {
  ...
  const FormPengubahanBahan(
      {Key? key, this.materialTransformId, this.machineId, this.statusMtf})
      : super(key: key);

  @override
  State<FormPengubahanBahan> createState() => _FormPengubahanBahanState();
}
  ...
  void initState() {
    ...
    if (widget.materialTransformId != null) {
      firestore
          ...
          .doc(widget.materialTransformId)
          .get()
        ...
        if (documentSnapshot.exists) {
          ...
          setState(() {
            ...
            if (tanggalPengubahanFirestore != null) {
              _selectedDate =
                  (tanggalPengubahanFirestore as Timestamp).toDate();
            }
...
// Tambahkan metode async untuk mengambil data mesin
  ...
  void addOrUpdate() {
    final materialTransformBloc =
        BlocProvider.of<MaterialTransformsBloc>(context);
    final materialTransform = MaterialTransforms(
        id: '',
        catatan: catatanController.text,
        jumlahBarangGagal: int.tryParse(jumlahController.text) ?? 0,
        jumlahHasil: int.tryParse(jumlahHasilController.text) ?? 0,
        machineId: selectedKodeMesin ?? '',
        satuan: satuanController.text,
        satuanHasil: satuanJumlahHasilController.text,
        satuanTotalHasil: satuanTotalController.text,
        status: 1,
        statusMtf: statusController.text,
        tanggalPengubahan: _selectedDate ?? DateTime.now(),
        totalHasil: int.tryParse(totalPengubahanController.text) ?? 0);

    if (widget.materialTransformId != null) {
      materialTransformBloc.add(UpdateMaterialTransformsEvent(
          widget.materialTransformId ?? '', materialTransform));
    } else {
                          ...
                          children: [
                            Expanded(
                              child: MachineDropdown(
                                selectedMachine: selectedKodeMesin,
                                onChanged: (newValue) {
                                  setState(() {
                                ...
                                title: 'Penggiling',
                                namaMesinController: namaMesinController,
                                isEnabled: widget.statusMtf != "Selesai",
                              ),
                            ),
                            const SizedBox(width: 16.0),
                          ...
                          children: [
                            Expanded(
                              child: ElevatedButton(
                                onPressed: widget.statusMtf == "Selesai"
                                    ? null
                                    : () {
