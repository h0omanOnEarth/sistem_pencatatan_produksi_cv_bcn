...
class FormFakturPenjualanScreen extends StatefulWidget {
  ...
  const FormFakturPenjualanScreen(
      {Key? key, this.invoiceId, this.shipmentId, this.statusFk})
      : super(key: key);
...
class _FormFakturPenjualanScreenState extends State<FormFakturPenjualanScreen> {
    ...
    for (final doc in snapshot.docs) {
      ...
      final customCard = CustomCard(
        content: [
          ...
          CustomCardContent(text: 'Jumlah (Pcs): ${jumlahPcs.toString()}'),
          CustomCardContent(text: 'Jumlah (Dus): ${jumlahDus.toString()}'),
          CustomCardContent(
              text: 'Harga per Pcs: Rp ${productPrice.toInt().toString()}'),
          CustomCardContent(text: 'Subtotal: Rp ${subtotal.toString()}'),
        ],
      );
  ...
  void initializeShipment() async {
    ...
    firestore
        .collection('shipments')
        .where('id',
            isEqualTo: widget.shipmentId) // Gunakan .where untuk mencocokkan ID
        .get()
      ...
      if (querySnapshot.docs.isNotEmpty) {
        ...
        Map<String, dynamic>? deliveryOrder = await deliveryOrderService
            .getDeliveryOrderInfo(shipmentData['delivery_order_id']);
  ...
  void initState() {
    ...
    if (widget.invoiceId != null) {
      firestore
          .collection('invoices')
          .doc(widget.invoiceId) // Menggunakan widget.customerOrderId
          .get()
  ...
  void _updateTotal() async {
    ...
    QuerySnapshot querySnapshot = await FirebaseFirestore.instance
        .collection('shipments')
        .doc(
            selectedNomorSuratJalan) // Ganti dengan ID dokumen shipment yang sesuai
        .collection('detail_shipments')
        .get();
    ...
    for (QueryDocumentSnapshot document in querySnapshot.docs) {
      ...
      Map<String, dynamic>? product =
          await productService.fetchProductInfo(detail['product_id']);
  ...
  void addOrUpdate() {
    ...
    final invoice = Invoice(
        ...
        nomorRekening: selectedNomorRekening,
        shipmentId: selectedNomorSuratJalan ?? '',
        status: 1,
        statusFk: statusController.text,
        total: total,
    ...
    for (var productCardData in materialDetailsData) {
      final detailInvoice = DetailInvoice(
          id: '',
          invoiceId: '',
          productId: productCardData['productId'],
          harga: productCardData['harga'],
          jumlahPengiriman: productCardData['jumlahPcs'],
          jumlahPengirimanDus: productCardData['jumlahDus'],
          subtotal: productCardData['subtotal'],
          status: 1);
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        DropdownWidget(
                          ...
                          selectedValue:
                              selectedMetodePembayaran, // Isi dengan nilai yang sesuai
                          ...
                          children: [
                            Expanded(
                              child: ElevatedButton(
                                onPressed: widget.statusFk == "Selesai"
                                    ? null
                                    : () {
