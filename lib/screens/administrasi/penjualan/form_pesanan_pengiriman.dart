...
class FormPesananPengirimanScreen extends StatefulWidget {
  ...
  const FormPesananPengirimanScreen(
      {Key? key, this.deliveryOrderId, this.customerOrderId, this.statusDo})
      : super(key: key);
...
class _FormPesananPengirimanScreenState
    extends State<FormPesananPengirimanScreen> {
  ...
  List<Map<String, dynamic>> customerOrderData =
  ...
  @override
  void dispose() {
    selectedCustomerOrderNotifier
        .removeListener(_selectedCustomerOrderListener);
  ...
  void addOrUpdateData() {
    ...
    try {
      final deliveryOrder = DeliveryOrder(
          id: '',
          customerOrderId: selectedPesanan ?? '',
          metodePengiriman: selectedMetode,
          satuan: 'Pcs',
          status: 1,
          ...
          totalHarga: int.tryParse(
              ...
              0,
          estimasiWaktu: int.tryParse(waktuPengirimanController.text) ?? 0,
          detailDeliveryOrderList: []);
      ...
      if (widget.deliveryOrderId != null) {
        //berarti update
        deliveryOrderBloc.add(UpdateDeliveryOrderEvent(
            widget.deliveryOrderId ?? '', deliveryOrder));
      } else {
  ...
  void fetchDataCustomerOrder() {
    ...
    firestore.collection('customer_orders').get().then((querySnapshot) {
      for (var doc in querySnapshot.docs) {
        ...
        setState(() {
          customerOrderData
              .add(customerOrder); // Tambahkan produk ke daftar produk
        });
      }
    });
  }
  ...
  Future<void> filterProductData(String customerOrderId) async {
    ...
    if (customerOrderId.isNotEmpty) {
      ...
      final detailCustomerOrders =
          detailCustomerOrderQuery.docs.map((doc) => doc.data());
  ...
  void initializeCustomerOrder() {
    ...
    firestore
        .collection('customer_orders')
        .where('id',
            isEqualTo: selectedPesanan) // Gunakan .where untuk mencocokkan ID
        .get()
  ...
  void fetchDataDetailDeliveryOrder() {
    firestore
        ...
        .collection(
            'detail_delivery_orders') // Ganti dengan nama collection yang sesuai
        .get()
        .then((querySnapshot) {
        ...
        if (documentSnapshot.exists) {
          ...
          setState(() {
            ...
            if (tanggalDeliveryOrderFirestore != null) {
              _selectedDate =
                  (tanggalDeliveryOrderFirestore as Timestamp).toDate();
            }
            final tanggalRequestFirestore = data['tanggal_request_pengiriman'];
            if (tanggalRequestFirestore != null) {
              _selectedReqDate =
                  (tanggalRequestFirestore as Timestamp).toDate();
            }
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        CustomerOrderDropDownWidget(
                            namaPelangganController: pelangganController,
                            alamatPengirimanController: alamatController,
                            isEnabled: widget.deliveryOrderId == null,
                            customerOrderId: widget.customerOrderId,
                            onChanged: (newValue) {
                              setState(() {
                          ...
                          children: [
                            Expanded(
                              child: ElevatedButton(
                                onPressed: widget.statusDo == "Selesai"
                                    ? null
                                    : () {
