...
Future<void> filterProductDataBahan(String materialRequestId) async {
  ...
  if (materialRequestId.isNotEmpty) {
    ...
    final detailMaterialRequests = detailMaterialRequestsQuery.docs.map((doc) => doc.data());
...
void initializeProductionOrder(){
    ...
    .where('id', isEqualTo: selectedNomorPerintah) // Gunakan .where untuk mencocokkan ID
    .get()
...
void addOrUpdate(){
 final materialUsageBloc = BlocProvider.of<MaterialUsageBloc>(context);
  ...
  for (var productCardData in productCards) {
    final detailMaterialUsage = DetailMaterialUsage(id: '', jumlah: int.tryParse(productCardData.jumlah)??0, materialId: productCardData.kodeBahan, materialUsageId: '', satuan: productCardData.satuan, status: 1);
