...
void initializeMaterialUsage(){
 ...
 @override
  ...
  void addOrUpdate(){
     ...
     final productionResult = ProductionResult(id: '', materialUsageId: selectedPenggunaanBahan??'', totalProduk: int.tryParse(jumlahProdukController.text)??0, jumlahProdukBerhasil: int.tryParse(jumlahProdukBerhasilController.text)??0, jumlahProdukCacat: int.tryParse(jumlahProdukCacatController.text)??0, satuan: selectedSatuan, catatan: catatanController.text, statusPRS: statusController.text, status: 1, tanggalPencatatan: _selectedDate?? DateTime.now(), waktuProduksi: int.tryParse(waktuProduksiController.text)??0);
     if(widget.productionResultId!=null){
