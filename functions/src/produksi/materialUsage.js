...
exports.materialUsageValidation = async (req) => {
  const {materials, materialRequestId, productionOrderId, batch, mode} = req.data;
  ...
  if (materialRequestSnapshot.exists) {
    ...
    if (materialRequestData.production_order_id !== productionOrderId) {
      return { success: false, message: `Nomor perintah produksi tidak sesuai dengan nomor permintaan bahan, seharusnya ${materialRequestData.production_order_id}`};
    }
  } else {
  ...
  const materialTransferQuery = await admin.firestore().collection("material_transfers")
    .where("material_request_id", "==", materialRequestId)
    .where("status_mtr", "==", "Selesai")
    .get();

  if (materialTransferQuery.empty) {
    return {success: false, message: "Bahan-bahan pada material request belum dipindahkan"};
  }

  if(mode=='add'){
    // Periksa apakah ada material_transfer yang memiliki production_order_id dan batch yang sama
    const materialUsagesQuery = await admin.firestore().collection("material_usages")
    .where("production_order_id", "==", productionOrderId)
    .where("batch", "==", batch).where("material_request_id","==", materialRequestId)
    .get();

    if (!materialUsagesQuery.empty) {
    return {success: false, message: "Material usage dengan production_order_id, material_request_id, dan batch yang sama sudah ada"};
    }
  }
  ...
  const detailMaterialRequestRef = admin.firestore().collection("material_requests").doc(materialRequestId).collection("detail_material_requests");
  const detailMaterialRequestQuery = await detailMaterialRequestRef.where("batch", "==", batch).get();

  if (detailMaterialRequestQuery.empty) {
    return { success: false, message: "Batch tidak sesuai dengan Material Request" };
  }

  // Pemeriksaan apakah material.material_id ada yang cocok dengan material_id pada detailMaterialRequestRef
  const detailMaterialRequests = detailMaterialRequestQuery.docs.map((doc) => doc.data());
  const invalidMaterialIds = materials.filter((material) => {
    return !detailMaterialRequests.some((detailMaterial) => detailMaterial.material_id === material.material_id);
  });

  if (invalidMaterialIds.length > 0) {
    const invalidMaterialIdList = invalidMaterialIds.map((material) => material.material_id).join(", ");
    return { success: false, message: `Material_id tidak valid: ${invalidMaterialIdList}` };
  }
