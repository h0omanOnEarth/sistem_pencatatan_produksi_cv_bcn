...
exports.materialTransferValidation = async (req) => {
    ...
    const materialRequestRef = admin.firestore().collection("material_requests").doc(materialRequestId);
    ...
    for (const material of materials) {
        ...
        if (!materialQuery.empty) {
            ...
            return {success: false, message: `Stok material '${materialId}' tidak mencukupi`};
            }
        } else {
            return {success: false, message: `Material '${materialId}' tidak ditemukan`};
        }
    }
    ...
    return {success: false, message: "Terjadi kesalahan dalam validasi material transfer"};
    }
}