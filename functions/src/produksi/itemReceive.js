...
exports.itemReceiveValidation = async (req) => {
    ...
    try {
      ...
      const productionConfirmationRef = admin.firestore().collection("production_confirmations").doc(productionConfirmationId);
      ...
      const detailProductionConfirmationsRef = productionConfirmationRef.collection("detail_production_confirmations");
      const detailProductionConfirmationsQuery = await detailProductionConfirmationsRef.get();
      ...
      return { success: false, message: "Terjadi kesalahan dalam validasi penerimaan item" };
    }
}