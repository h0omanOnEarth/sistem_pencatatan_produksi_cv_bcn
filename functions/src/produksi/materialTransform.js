...
exports.materialTransformValidate = async (req) => {
  ...
  if (!jumlahBarangGagal || isNaN(jumlahBarangGagal) || jumlahBarangGagal < 0) {
    return {
      success: false,
      message: "Jumlah barang gagal harus lebih besar dari 0",
    };
  }
  ...
  try {
    ...
    if (stokProductXXX - jumlahBarangGagal < 0) {
      return {
        success: false,
        message: `Stok produk gagal tidak mencukupi, stok saat ini\n ${stokProductXXX}`,
      };
    }
    ...
    return {
      success: false,
      message: "Terjadi kesalahan dalam validasi transformasi material",
    };
  }
};
