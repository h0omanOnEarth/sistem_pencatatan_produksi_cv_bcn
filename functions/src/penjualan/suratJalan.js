...
async function generateNextNotificationId() {
  ...
  while (status) {
    nextNotificationId = `NOTIF${notificationCount
      .toString()
      .padStart(5, "0")}`;
...
exports.suratJalanValidation = async (req) => {
  ...
  if (
    ...
    return {
      success: false,
      message: "Jumlah pengiriman pada detail harus di atas 0",
    };
  }
  ...
  if (
    ...
    return {
      success: false,
      message: "Jumlah pengiriman dus pada detail harus di atas 0",
    };
  }
  ...
  try {
    ...
    const deliveryOrdersRef = admin
      .firestore()
      .collection("delivery_orders")
      .doc(deliveryOrderId);
    const deliveryOrderDoc = await deliveryOrdersRef.get();
    const statusPesananPengiriman =
      deliveryOrderDoc.data().status_pesanan_pengiriman;
    ...
    const notificationsRef = admin.firestore().collection("notifications");
    const detailDeliveryOrdersRef = deliveryOrdersRef.collection(
      "detail_delivery_orders"
    );
    ...
    return {
      success: false,
      message: "Terjadi kesalahan dalam validasi surat jalan",
    };
  }
};
