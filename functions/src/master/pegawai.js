...
exports.pegawaiAdd = async (req) => {
  const { username, email, password, telp, gajiHarian, gajiLembur, status } =
    req.data;
...
exports.pegawaiUpdate = async (req) => {
  const { username, telp, gajiHarian, gajiLembur, status, currentUser } =
    req.data;
  ...
  if (!qSnapUname.empty) {
    ...
    if (
      ...
      existingUsers[0].data().username != currentUsername
    ) {
      return { success: false, message: "Username telah digunakan" };
    }
  }
